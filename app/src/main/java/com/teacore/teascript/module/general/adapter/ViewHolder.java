package com.teacore.teascript.module.general.adapter;
...
public class ViewHolder {
    ...
    //初始化ViewHolder，需要传递进实现了ViewHolder.Callback接口的类
    public ViewHolder(Callback callback, ViewGroup parent,int layoutId,int position){
        ...
        LayoutInflater inflater=callback.getInflater();
    ...
    public static ViewHolder getViewHolder(Callback caller, View convertView, ViewGroup parent, int layoutId, int position) {
        ...
        ViewHolder vh = null;
        //如果convertview不为null，convertview.getTag获取相应的viewholder
        if (convertView == null) {
            needCreateView = true;
        } else {
        ...
        //如果needCreateView为true，通过构造函数返回新的ViewHolder
        if (needCreateView) {
    ...
    public <T extends View> T getView(int viewId) {
        ...
        //如果SparseArray中获取的View为null
        if (view == null) {
            view = mConvertView.findViewById(viewId);
            //将获取的view放入到SparseArray中
    ...
    public void setImageFromNet(int viewId, String imgUrl, int emptyRes) {
        ImageView iv = getView(viewId);
        RequestManager loader = mCallback.getImageLoader();
        GlideImageLoader.loadImage(loader, iv, imgUrl, emptyRes);
    }

    public void setImageFromNet(int viewId, String imgUrl) {
        setImageFromNet(viewId, imgUrl, R.drawable.bg_normal);
    }
