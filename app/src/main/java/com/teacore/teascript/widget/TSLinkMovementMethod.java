...
public class TSLinkMovementMethod extends LinkMovementMethod{

    private static TSLinkMovementMethod tsLinkMovementMethod;
    private SmallCardSpan a;
    private ClickableSpan b;
    private int c;

    public TSLinkMovementMethod() {
        c = 90;
    }

    public static MovementMethod getMovementMethod() {
        if (tsLinkMovementMethod == null)
            tsLinkMovementMethod = new TSLinkMovementMethod();
        return tsLinkMovementMethod;
    }
    ...
    public boolean onTouchEvent(TextView textview, Spannable spannable,
        ...
        if (i == 0) {
            ...
            SmallCardSpan aam1[] = spannable.getSpans(l2, l2, SmallCardSpan.class);
            ClickableSpan aclickablespan1[];
            ClickableSpan clickablespan1;
            if (aam1 != null && aam1.length != 0) {
                SmallCardSpan am2 = aam1[0];
            ...
            if (a == null) {
                ...
                SmallCardSpan aam[] = (SmallCardSpan[]) spannable.getSpans(j2, j2, SmallCardSpan.class);
                if (aam != null && aam.length != 0) {
                    SmallCardSpan am1 = aam[0];
